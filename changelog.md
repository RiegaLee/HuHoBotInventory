...
- 新增与 Faithful 物品资源解耦的自定义底图层：服主可为背包和末影箱分别放置本地 PNG，或让两者复用同一图片。
- 自定义底图采用桌面式合成：壁纸按画布圆角裁切，每个物品位置与人物区域单独绘制薄荷喵式半透明圆角选择框，再绘制图标与人物；不会把原主题底板整张覆盖到壁纸上。
- 背包与末影箱直接采用薄荷喵布局基准：背包 `1359×1017`、104 像素槽位与 96 像素图标，末影箱 `1620×694`、128 像素槽位与 112 像素图标；人物框同步改为 `272×373`，避免旧布局中的人物偏矮。
- 取消 Faithful 与 CozyUI+ 背包底板分支，UI 统一为桌面式圆角布局；Faithful 仅继续提供物品、特殊物品和头颅贴图。未配置用户底图时使用项目原创的 `1359×1017` 雾蓝像素壁纸。
- 支持保持比例居中裁切的 `cover` 与铺满画布的 `stretch`，并限制文件名、文件大小和解码像素数。
- QQ 查询入口统一为 `/我的背包`、`/我的末影箱`；删除公开的渲染测试指令与英文别名，避免无用入口出现在指令面板。
- 配置版本升级到 12，旧配置自动补齐关闭状态的底图项；升级时不会擅自启用用户底图，未配置时使用内置雾蓝壁纸。
- 未知物品不再显示类似幸运方块的紫色问号箱，改为项目原创的完整 3D 紫黑马赛克方块；该图标未从游戏本体或资源包解包。本地渲染预览也强制使用与正式包一致的 64×64 生成物品图标。
- README 明确本 Addon 应搭配 HuHoBot-Penguin 1.3.0 的 Spigot / Paper 版；项目许可证改为与上游一致的 GNU AGPL v3.0。
...
- 修复复用官方 HuHoBot 宿主时仍从旧宿主上下文查询绑定的问题；背包与末影箱现在动态读取 GameAuthCode 注册的当前绑定服务，绑定成功后可立即查询。
- 配置版本升级到 14；旧配置会自动移除测试命令配置，并把玩家入口迁移为无英文别名的 `/我的背包` 与 `/我的末影箱`。
...
- 背包与末影箱账号选择按钮在点击、新一轮操作或 60 秒超时后撤回，避免过期按钮长期留在群内。

## 1.22.0 — 2026-09-20

- 从薄荷喵 RC21 回移 Faithful 特殊物品渲染：时钟、指南针、追溯指针和六种生物头颅均使用独立贴图，不再落入问号图标或共享玩家头近似图。
- 玩家头颅会从 Paper 物品档案捕获 Mojang 材质哈希，按单个物品异步下载、缓存并渲染；只有缺少可用身份时才回退到 Faithful Steve。
- 修正头颅 GUI 朝向与 UV，避免镜像后把后脑勺边缘画到正面；Dragon 与 Piglin 使用各自模型部件而非普通立方体替代。
- 离线背包/末影箱快照升级到 schema 4，持久化玩家头颅材质或拥有者身份；schema 1—3 仍可读取。
- 玩家预览升级到 PV13：按深度排序皮肤与盔甲面，收紧盔甲膨胀，适配细手臂，并避免被盔甲覆盖的皮肤第二层穿模。
- 块模型改为逐像素深度合成，改善蜂蜜块等嵌套半透明模型；物品数量字号同步采用薄荷喵当前值。
- Faithful 与 CozyUI+ 主题资源及来源清单同步更新；资源包标识升级为 `inventory-assets-v14-mb7-pv13-faithful-special-heads`。
- 126 项自动测试、完整构建、插件 JAR 边界检查及 HuHoBot 原版 Paper 1.21.11 测试服启停通过。
...
- 补充中文 README、源码许可证、第三方资源声明和 Gradle Wrapper。
...
- Faithful 32x 资源继续适用独立 Faithful License，不纳入项目源码许可证；JAR 内保留许可证与精确来源记录。
